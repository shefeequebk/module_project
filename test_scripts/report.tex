...
\setcounter{page}{1}    % Ensure first content page is page 1
...
The student (myself) is always one of the registered people and holds my student ID card.
At least one person leaves and later returns to the scene during each recording.

The system combines two main components: (i) a TensorFlow Lite object detector based on SSD
MobileNet running on the Raspberry Pi, and (ii) a face recognition pipeline using the
\texttt{face\_recognition} library.
I started development on my laptop using a webcam for easier debugging and then deployed the
final version to the Raspberry Pi~4 using the OKDO 5MP camera with \texttt{BGR888} capture format.
...
The hardware platform is a Raspberry Pi 4 with a Raspberry Pi camera.
The main software libraries used are OpenCV for image processing, Picamera2 for camera
control, \texttt{face\_recognition} for face feature extraction, and TensorFlow Lite for
object detection \cite{vigneshLab3,vigneshLab5}.
The design of the system is guided by content and experiments from the labs of the module covering image sensors, ISP pipelines.
\cite{dennyISP}.

In the project, I implemented a Python script that captures camera frames, resizes them for processing,
 runs face recognition and object detection, draws the results on the image,
  and saves a plot of detections over time. 
...
\begin{itemize}
    ...
    \item OKDO 5MP camera module.
    \item Tripod for the camera, aimed at an area where people can move in and out.
    \item An object (a \emph{cell phone}) that is visible in the scene.
\end{itemize}

Figure~\ref{fig:setup} shows the hardware setup, including the student ID card.

\begin{figure}[H]
    \centering
    % Replace placeholder image name with your own file, e.g. setup_photo.jpg
    \IfFileExists{fig_setup_placeholder.jpg}{%
        \includegraphics[width=0.6\linewidth]{fig_setup_placeholder.jpg}%
    }{%
        \fbox{\parbox{0.8\linewidth}{\centering\vspace{2cm}\textcolor{gray}{[Image: fig\_setup\_placeholder.jpg\\Place your setup photo here]\vspace{2cm}}}}%
    }
    \vspace{-0.3cm}
    \caption{\footnotesize Raspberry Pi~4 and camera setup for the experiment.\label{fig:setup}}
\end{figure}

I initially developed and debugged the code on my laptop by using webcam instead of the
OKDO 5MP camera. The script includes a \texttt{--webcam} flag that switches between the Picamera2 and
OpenCV's \texttt{VideoCapture(0)}. Once the pipeline was stable on the laptop, I cloned the
GitHub repository on the Raspberry Pi and ran the training and inference completely on the Raspberry Pi.
...
\begin{enumerate}
    \item Captures frames from the OKDO 5MP camera in \texttt{BGR888} format.
    ...
    \item Draws detection bounding boxes on the frame.
...
The Raspberry~Pi~4 runs Raspberry Pi OS (64-bit) with Python~3. The OKDO 5MP camera is configured
for the project. I used the following configuration in the final script:
...
config = picam2.create_preview_configuration(
    main={"format": "BGR888",  "size": (imW, imH)},
...
For each individual, I collected at least twelve photos and saved them in the \texttt{faces/} directory.
...
Raspberry Pi repository \cite{armaanTFLite} and Lab-5 materials \cite{vigneshLab5}. The model and label file are loaded once:
...
handled separately by the face recognition pipeline, and I kept only \emph{cell phone} for the object detection.
...
In each case, at least four people and one object (\emph{mobile phone}) were visible.
One person intentionally left the scene during the 20~seconds.

\subsection{Overall Behaviour}

Figures~\ref{fig:timeline1},~\ref{fig:timeline2}, and~\ref{fig:timeline3} show plots of detection confidence over time for the different labels
in each of the three scenarios. Each coloured curve corresponds to a person or object.
Long gaps in a person's curve indicate intervals where the face was not detected or recognised.

\begin{figure}[H]
    \centering
    \IfFileExists{fig_detections_scenario1.jpg}{%
        \includegraphics[width=0.8\linewidth]{fig_detections_scenario1.jpg}%
    }{%
        \fbox{\parbox{0.9\linewidth}{\centering\vspace{2cm}\textcolor{gray}{[Image: fig\_detections\_scenario1.jpg\\Place your confidence plot for Scenario 1 here]\vspace{2cm}}}}%
    }
    \vspace{-0.5cm}
    \caption{\footnotesize Confidence over time for different people and objects in Scenario~1: Bright indoor room.\label{fig:timeline1}}
\end{figure}

\begin{figure}[H]
    \centering
    \IfFileExists{fig_detections_scenario2.jpg}{%
        \includegraphics[width=0.8\linewidth]{fig_detections_scenario2.jpg}%
    }{%
        \fbox{\parbox{0.9\linewidth}{\centering\vspace{2cm}\textcolor{gray}{[Image: fig\_detections\_scenario2.jpg\\Place your confidence plot for Scenario 2 here]\vspace{2cm}}}}%
    }
    \vspace{-0.5cm}
    \caption{\footnotesize Confidence over time for different people and objects in Scenario~2: Outdoor overcast daytime.\label{fig:timeline2}}
\end{figure}

\begin{figure}[H]
    \centering
    \IfFileExists{fig_detections_scenario3.jpg}{%
        \includegraphics[width=0.8\linewidth]{fig_detections_scenario3.jpg}%
    }{%
        \fbox{\parbox{0.9\linewidth}{\centering\vspace{2cm}\textcolor{gray}{[Image: fig\_detections\_scenario3.jpg\\Place your confidence plot for Scenario 3 here]\vspace{2cm}}}}%
    }
    \vspace{-0.5cm}
    \caption{\footnotesize Confidence over time for different people and objects in Scenario~3: Indoor low light with strong backlighting.\label{fig:timeline3}}
\end{figure}
...
\begin{quote}
\small
\texttt{- Shefeeque (face):}\\
\texttt{  from 6.2s to 6.2s (duration 0.0s)}\\
\texttt{  from 8.7s to 8.8s (duration 0.1s)}\\
\texttt{- cell phone (object):}\\
\texttt{  from 1.9s to 1.9s (duration 0.0s)}\\
\texttt{  from 3.8s to 4.9s (duration 1.1s)}
\end{quote}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|p{4.5cm}|p{4.5cm}|}
    \hline
    Scenario & Avg.\ FPS & Face recognition quality & Object detection quality \\
    \hline
    Bright indoor & $\approx$ 1--3 & Stable confidence ($\geq 0.5$), higher for faces, $\approx 0.5$ for cell phone & Cell phone detected with $\approx 0.5$ confidence, false positives present \\
    \hline
    Outdoor daytime & $\approx$ 2--3 & Less stable, lower than Scenario~1 & Lower confidence. \\
    \hline
    Low light / backlit & $\approx$ 2--3 & Very low confidence, highly fluctuating & Cell phone detected only for short periods, very fluctuating, false positives present \\
    \hline
    \end{tabular}
\end{table}
...
In the bright indoor scenario, the camera sensor receives a strong signal and the ISP can keep
gain low, so noise is limited. Face recognition confidence is stable, typically around 0.5 or higher.
Faces generally achieve higher confidence values than the cell phone object, which typically
maintains confidence around 0.5. The system runs at approximately 1--3 FPS.

The \texttt{detections\_log} showed stable detection intervals for faces throughout the 20~seconds.
The cell phone object was detected with consistent confidence around 0.5.

\subsection{Scenario 2: Outdoor Overcast Daytime}

Outdoors with soft natural light, the system achieved similar frame rates (approximately 2--3 FPS)
as Scenario~1. However, confidence values were less stable and generally lower than in the bright
indoor scenario. The auto-exposure and auto white-balance in the ISP had to adjust more often
\cite{dennyISP}, which contributed to the reduced stability in confidence measurements.

The confidence plot shows more variation compared to Scenario~1, with confidence values fluctuating
more noticeably.
...
The system maintained approximately 2--3 FPS, similar to the other scenarios.

According to the image sensor theory, increasing analog gain in low light amplifies both
signal and noise, reducing the effective signal-to-noise ratio.
This explains why face detection and recognition confidence was very low and highly
fluctuating during these periods. The cell phone object was detected only for short
periods, with very unstable confidence values and sometimes faces were not detected at all for several frames,
which appears as gaps in the confidence plot.
...
\begin{itemize}
    ...
    \item \textbf{Auto white-balance:}
          Sudden colour temperature changes can temporarily distort skin tones until the AWB settles.
\end{itemize}
...
Face recognition systems raise important ethical questions. All people in my experiments gave consent to be recorded,
 and the system was used only for this coursework. In real use, clear consent and data protection policies would be needed.
  Face encodings are stored in a file (\texttt{encodings.pickle}),
   which should be encrypted and access controlled in a real deployment. A small training set with only a few people
    can lead to bias, a real system would require much more diverse training data to avoid systematic errors.
...
training and inference fully on the Raspberry~Pi using the OKDO 5MP camera and TensorFlow Lite.
This experiments showed how careful control of lighting and camera configuration is essential for reliable embedded vision systems.

Possible future improvements include better automatic quality feedback (for example, changing
camera exposure settings when average brightness is too low), more robust tracking of people
...
\bibitem{dennyISP}
T.~V.~Vignesh.
\newblock CS6461-- Computer Vision Systems.
\newblock Lab-4: Image Signal Processing (ISP) vs Real-Time Face Recognition Performance
\newblock Images, CS6461 Teaching Assistant.
\newblock University of Limerick, 2025.

\bibitem{vigneshLab3}
T.~V.~Vignesh.
\newblock CS6461-- Computer Vision Systems.
\newblock Lab-3: Real-Time Face Recognition.
\newblock CS6461 Teaching Assistant.
\newblock University of Limerick, 2025.

\bibitem{vigneshLab5}
T.~V.~Vignesh.
\newblock CS6461-- Computer Vision Systems.
\newblock Lab-5: Object Detection with TensorFlow Lite on JPG Images.
\newblock CS6461 Teaching Assistant.
\newblock University of Limerick, 2025.

\bibitem{picamera2docs}
OKDO.
\newblock OKDO 5MP camera documentation.
\newblock Available at: \url{https://docs.rs-online.com/b064/A700000006917308.pdf}.
Accessed: 2025.
