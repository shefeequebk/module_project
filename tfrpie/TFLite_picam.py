...
picam2 = Picamera2()
picam2.configure(picam2.create_preview_configuration(main={"format": 'XRGB8888', "size": (imW, imH)}))
picam2.start()
time.sleep(1) # Allow camera to warm up
#---
cv2.namedWindow('Face Rec Running', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Face Rec Running', 1280, 720) # Setting Default
#---
...
# Start timer (for calculating frame rate)
    ...
    # The frame from picamera2 is in RGBA or RGB format, so we might need to convert it to BGR for OpenCV drawing
    frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR) # Or COLOR_RGB2BGR if format is XRGB8888

    # Acquire frame and resize to expected shape [1xHxWx3]
    # No need for frame.copy() anymore, but it doesn't hurt
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    frame_resized = cv2.resize(frame_rgb, (width, height))
    input_data = np.expand_dims(frame_resized, axis=0)
